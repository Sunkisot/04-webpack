  ...
  <div class="wrapper pd10">
    <gv-search-table ref="table" :table="table" @on-list-btn="onListBtn">
      <template v-slot:search="scope">
        ...
        <el-row>
          <gv-form-item keyName="成员角色" prop="roleId">
            <gv-select
              url="/system/mechanism/role/optionList"
              codeName="id"
              labelName="name"
              placeholder="选择成员角色"
              :searchObject="{ mechanismId: scope.search.mechanismId }"
      ...
      <template v-slot:toolbar>
        <el-button type="primary" @click="onHandleAdd()" v-hasPermi="['system:mechanism:user:save']">添加成员</el-button>
      </template>
    </gv-search-table>
  </div>
</template>
...
<script>
import { getUserStatus, delUser } from "@/api/orgManage";
export default {
  name: "memb-index",
  data(vm) {
    return {
      table: {
        basic: {
          path: "/system/mechanism/user/list", //分页列表请求api
          vo: "list", //分页列表返回的vo
        ...
        fields: [
          //结果列表配置，一个对象一列
          {
            labelKey: "成员姓名", //国际化key
            btns: [
              {
                prop: "name",
                flag: "edit",
                type: "a",
                perms: ['system:mechanism:user:update']
              },
            ],
          },
          {
                ...
                ddItems: [
                  {
                    command: "start",
                    name: "启用",
                    perms: ['system:mechanism:user:disableOrEnable']
                  },
                  {
                    command: "stop",
                    name: "禁用",
  ...
  methods: {
    ...
    onHandleAdd() {
      this.$router.push({
        path: "/org-manage/memb-detail/add",
      });
    },
    ...
    handleEdit(row) {
      this.$router.push({
        path: "/org-manage/memb-detail/edit",
        query: {
          id: row.id,
        },
      });
