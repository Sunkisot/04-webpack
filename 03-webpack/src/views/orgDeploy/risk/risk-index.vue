...
<script>
import { getRiskDetail, postRiskAdd, getRiskOpenSync } from "@/api/orgDeploy";
import { getOrgSonTree } from "@/api/orgManage";
import RiskForm from "./components/risk-form.vue";
export default {
  name: "risk-index",
  components: {
    RiskForm,
  ...
  methods: {
    ...
    async proFormData(e) {
      this.proIsTopOrg();
      let res = await getRiskDetail(e.id);
      if (res && res.code == "00000") {
        if (res.data) {
