...
export default {
  ...
  data() {
    return {
      ...
      filterText: "", // 筛选字短
      type: "edit", // 页面类型 detail 详情 edit 编辑 add 添加
      oldkey: "",
      oldNode: null, // 上一级节点
      formData: {},
      oldFormData: {},
      initForm: {
        ...
        isShowDeleted: 0,
        isOpenApprove: 0,
        approveMechanism: "AGENT",
        supportPlate: [],
        isOpenQueryPlate: 1,
        isNeedIdCard: 1,
        isOpenQueryVin: 1,
        isOpenQueryPolicy: 1,
        queryWay: ["MODEL", "CHECK"],
        defaultQuery: "MODEL",
        maxQuotaIc: null,
        isOpenTemp: 0,
        isShowCost: 0,
        ...
        isOpenArtificial: 0,
        isNeedChoose: 0,
        artificialQuotaPrompt: null,
        artificialSupportBank: [],
        taxManualRange: [0, 99],
  ...
  methods: {
    ...
    async postOpenSync() {
      ...
      let res = await getMcFeatureSync({
        mechanismId: this.$refs.tree.getCurrentKey(),
        parentMechanismId: node.parentId,
      });
      if (res && res.code == "00000") {
        let res = await getMcFeatureDetail(node.parentId);
        if (res && res.code == "00000") {
    ...
    async handleSave() {
      ...
      let res = await postMcFeature(data);
      if (res && res.code == "00000") {
        this.gvUtil.success();
        this.refreshPage();
      } else {
    ...
    async proFormData(e) {
      this.proIsTopOrg();
      let res = await getMcFeatureDetail(e.id);
      if (res && res.code == "00000") {
